...
| [vimeo](https://github.com/juliandickie/vimeo-plugin) | Push Descript exports from Google Drive into your Vimeo library - multi-language captions and subtitles, title and description sync, new-video uploads with folder placement, and a gated source-file replace. Driven by a Google Sheet manifest read through the Scribe plugin. Token via config field, 1Password secret reference, or config file. |
| [descript](https://github.com/juliandickie/descript-plugin) | Full programmatic access to the Descript API - import media, Underlord AI agent edits, caption translation with creation-time language mapping (regional variants supported), free synchronous transcript export in six formats, live model catalog, publish compositions, inspect jobs, and run bulk video pipelines. Node CLI wrapped by skills and an optional MCP shim. Supply your own Descript API token. |
...
| [modern-web-guidance](https://github.com/GoogleChrome/modern-web-guidance) | Modern web platform guidance for coding agents from the Chrome and Edge teams - 132 use-case guides across CSS, forms, performance, UI behaviours, visual design, accessibility, passkeys and security, built-in AI and WebMCP, each with Baseline browser-support data and a fallback, plus a Chrome extensions skill. Third-party by Google Chrome. Retrieves guides through the modern-web-guidance npm CLI at runtime; set DISABLE_TELEMETRY=1 to opt out of Google usage telemetry. |
| [youtube](https://github.com/juliandickie/youtube-plugin) | Read YouTube through the official Data API for research and channel work. Comments with replies fully resolved, most-discussed sweeps across any channel including competitors, metadata, transcripts, and voice-of-customer export shaped for an Ultimate Message Map, routed per client. CLI plus MCP tools over one core, with a local quota ledger. |
| [reddit](https://github.com/juliandickie/reddit-plugin) | Read Reddit through the official Data API for research and voice-of-customer mining. Search, full comment trees, subreddit browsing, account history, and export shaped for an Ultimate Message Map. CLI only, so nothing loads into context until a command runs. Needs your own Reddit API credentials. |
| [heyzine](https://github.com/juliandickie/heyzine-plugin) | Publish PDFs as Heyzine flipbooks and carry them to lead magnet share kits, LearnDash course material, bookshelves and access lists. A heyzine CLI (REST, JSON, unattended) plus an MCP bridge to Heyzine's hosted server, one key resolver (config field, config file, 1Password reference, or OAuth), and the publish register kept inside Heyzine as tags and a private note. |
| [rocket-net](https://github.com/juliandickie/rocket-net-plugin) | Manage Rocket.net WordPress hosting - site lifecycle, staging and deploys, WP-CLI passthrough, backups, cache, domains and SSL, users, plugins and themes, reporting and WAF. Rocket.net's hosted MCP bundled for conversational work, plus a zero-dependency Python CLI covering all 198 API operations with async task polling for scripted and bulk work. 12 skills and 5 commands, --yes guard on every destructive op. |
| [short-io](https://github.com/juliandickie/short-io-plugin) | Short.io link management under your own API key. An MCP bridge to Short.io's hosted server (14 tools) with one key resolver (config field, SHORT_IO_API_KEY, config file, or 1Password reference), plus a short-io CLI for full enumeration to the true total, destination checks that expose silent fallbacks, deduped bulk create from CSV with a dry run, and delete with read-back. |
